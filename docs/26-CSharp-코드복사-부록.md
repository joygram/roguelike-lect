# 26. C# 코드 복사 부록 (07 Phase 0~5 대응)

**파일명**: `26-CSharp-코드복사-부록.md` — [07](07-단계별-개발-가이드.md)과 **같은 Phase 번호**의 아래 섹션에 C# 전문이 모여 있습니다. **실습·점검의 정본은 항상 07**입니다.

## 이 파일을 따로 둔 이유

| 질문 | 답 |
|------|-----|
| **왜 단계별 문서가 하나 더 있나?** | **긴 C# 전문**을 **한 파일에만** 모아 **복사·검색·비교**하기 위해서입니다. 웹/인쇄에서 스크롤 한 번에 코드 블록을 옮기기 쉽게 하려는 **부록**입니다. |
| **단계 목표·에셋·절차의 정본은 어디?** | **[07](07-단계별-개발-가이드.md)** 각 Phase + **아래 표의 주제 문서**. 그쪽에 **목표·에셋·산출물**이 먼저 정리되어 있습니다. |
| **26을 안 열고 만들 수 있나?** | 가능합니다. 07·08·09·10…만으로 진행하고, 코드는 직접 쓰거나 각 문서의 절차·[16](16-Unity-API와-CSharp-구분-및-패턴-가이드.md)을 따르면 됩니다. |

**정본이 있는 문서 (목표·에셋·단계별 녹임)**

| Phase | 목표·에셋·절차가 **본문에** 있는 문서 |
|:-----:|----------------------------------------|
| 0 | [02](02-준비사항-체크리스트.md) §4, [07](07-단계별-개발-가이드.md) Phase 0 |
| 1 | [07](07-단계별-개발-가이드.md) Phase 1 |
| 2 맵 | [README #map-asset-pipeline](../README.md#map-asset-pipeline) → [08](08-데이터테이블-및-맵구현-가이드.md), [11](11-맵이벤트-구성-스크롤-리소스-가이드.md), [28](28-기획서-맵구성-에셋-구현-연동.md), [07](07-단계별-개발-가이드.md) Phase 2 |
| 3 전투 | [09](09-이펙트-몬스터-스킬-레벨-가이드.md), [14](14-점증적-시스템완성-및-스킬확장-가이드.md), [15](15-이펙트-구성-및-스킬-설계-구현-가이드.md), [30](30-보스-AI-설계-및-구현.md), [07](07-단계별-개발-가이드.md) Phase 3 |
| 4 UI | [10](10-UI-HUD-디자인-및-구현-가이드.md), [25](25-참조게임-UI-프리팹-디자인-레이아웃.md), [07](07-단계별-개발-가이드.md) Phase 4 |
| 5 | [12](12-사운드-이펙트-연결방안.md), [19](19-분야별-분리-통합-및-제품출시-가이드.md), [07](07-단계별-개발-가이드.md) Phase 5 |

**README 순서**: 10~15단계에서 **07 + 위 표 문서**를 본 뒤, 코드가 필요할 때만 아래 **Phase 0~5** 섹션을 펼칩니다.

코드 주석: **[Unity API]** · **[우리 구현]** · **[직렬화]**. 용어는 [16](16-Unity-API와-CSharp-구분-및-패턴-가이드.md), [22](22-용어-발음-정의-및-심화학습.md).

**팀·Git**: [27](27-데이터-코드-에셋-병렬개발-및-커밋.md). **Phase 열기 전**: [07](07-단계별-개발-가이드.md) 해당 Phase **시작 전/완료 후** 표.

**폴더**: [README 세 폴더](../README.md#folder-layers) — `_Framework`·`_Content`·`_UI`.
...
| **코드 복사 부록** | **26 (본 문서)** | 위 문서를 읽은 뒤 C#만 붙여넣을 때 |
...
**맵·기획서 장애물 흐름**: [README #map-asset-pipeline](../README.md#map-asset-pipeline) (에셋·구현·연동 정본). 프리팹·전체 C#은 [28](28-기획서-맵구성-에셋-구현-연동.md).
...
**File → Build Settings** → 씬 추가 → **PC** 타깃 → Build. 상세 체크는 [19-분야별-분리-통합-및-제품출시-가이드.md](19-분야별-분리-통합-및-제품출시-가이드.md).
...
**연동**: [07](07-단계별-개발-가이드.md) **동일 Phase**를 펼칠 때 사용. **시작 전**: 항상 [07](07-단계별-개발-가이드.md) 해당 Phase **시작 전 준비**·**이전 Phase 완료 후** 먼저 확인.
